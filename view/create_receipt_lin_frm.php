...
$servidor_route = $_SERVER['DOCUMENT_ROOT']."/logistic_corp";
    ...
    <input type="hidden" name="indicador" >
        ...
        <div class="input-group input-group-sm mb-3">
            ...
            <select class="form-select" name="prtnum">
            <option value=""></option>
        ...
        <div class="col-md-2">
              <br>
              <a href="./create_receipt_frm.php?no_receipt=<?php echo $id ?>&indicador=CU"><div class="btn btn-sm btn btn-secondary form-control">Cancelar</div></a>
        </div>
        <?php 
        if($user_work == "" && $no_locc == ""){
        if($no_line != ""){
        ?>    
        <div class="col-md-2">
              <br>
              <div class="btn btn-sm btn-danger form-control" onclick="eliminar_receipt_lin()">Eliminar</div>
        </div>
        <?php
        }
        ?>
        <div class="col-md-2">
              <br>
              <div class="btn btn-sm btn-primary form-control" onclick="guardar_receipt_lin()">Guardar</div>
        </div>
        <?php
        }
        ?>
